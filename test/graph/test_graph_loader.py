...
def custom_rdf_graph_loader(pizza_bisou_kg_file_path) -> RDFGraphLoader:
    ...
    graph_loader = RDFGraphLoader(
        ...
        context_properties={"rdfs:comment", "http://www.msesboue.org/o/pizza-data-demo/bisou#context_prop"},
        lang_filter_tag="en"
    )
...
class TestDefaultRDFGraphLoaderInit:
    ...
    def test_attributes(self, default_rdf_graph_loader, pizza_bisou_kg_file_path) -> None:

        assert default_rdf_graph_loader.label_properties == {"rdfs:label"}
        assert default_rdf_graph_loader.context_properties is None
        assert str(default_rdf_graph_loader._kg_file_path) == str(pizza_bisou_kg_file_path)
...
class TestCustomRDFGraphLoaderInit:
    ...
    def test_attributes(self, custom_rdf_graph_loader, pizza_bisou_kg_file_path) -> None:

        assert custom_rdf_graph_loader.label_properties == {"rdfs:label", "<http://www.w3.org/2004/02/skos/core#altLabel>"}
        assert custom_rdf_graph_loader.context_properties == {"rdfs:comment", "<http://www.msesboue.org/o/pizza-data-demo/bisou#context_prop>"}
        assert str(custom_rdf_graph_loader._kg_file_path) == str(pizza_bisou_kg_file_path)

        splitted_label_sparql_alt_path_str = custom_rdf_graph_loader._label_sparql_alt_path_str.split("|")
        assert "rdfs:label" in splitted_label_sparql_alt_path_str
        assert "<http://www.w3.org/2004/02/skos/core#altLabel>" in splitted_label_sparql_alt_path_str

        splitted_context_sparql_alt_path_str = custom_rdf_graph_loader._context_sparql_alt_path_str.split("|")
        assert "rdfs:comment" in splitted_context_sparql_alt_path_str
        assert "<http://www.msesboue.org/o/pizza-data-demo/bisou#context_prop>" in splitted_context_sparql_alt_path_str

        assert custom_rdf_graph_loader._sparql_lang_filter_str == f'FILTER ( lang(?{custom_rdf_graph_loader._sparql_var}) = "en" )'
...
class TestSPARQLqueries:
    def test_build_ent_labels_sparql_query(self, default_rdf_graph_loader, custom_rdf_graph_loader) -> None:
        ...
        assert 'FILTER ( lang(?label) = "en" )' in custom_loader_query

        # test for the alternative paths
        assert "|" not in default_loader_query
        assert "|" in custom_loader_query

    def test_build_ent_context_from_labels_query(self, default_rdf_graph_loader, custom_rdf_graph_loader) -> None:
        ent_uri = "http://www.msesboue.org/o/pizza-data-demo/bisou#_godSaveTheKing"

        default_loader_query = default_rdf_graph_loader._build_ent_context_from_labels_query(ent_uri)
        custom_loader_query = custom_rdf_graph_loader._build_ent_context_from_labels_query(ent_uri)
    ...
    def test_build_ent_context_from_props_query(self, default_rdf_graph_loader, custom_rdf_graph_loader) -> None:
        ent_uri = "http://www.msesboue.org/o/pizza-data-demo/bisou#_godSaveTheKing"

        default_loader_query = default_rdf_graph_loader._build_ent_context_from_props_query(ent_uri)
        custom_loader_query = custom_rdf_graph_loader._build_ent_context_from_props_query(ent_uri)
...
class TestGetContext:
    ...
    def test_kg_get_context_default_loader(self, default_rdf_graph_loader) -> None:
        ...
        context_string = get_context("http://www.msesboue.org/o/pizza-data-demo/bisou#_burraTadah")
    ...
    def test_kg_get_context_custom_loader(self, custom_rdf_graph_loader) -> None:
        ...
        assert context_string == "The 'BurraTadah' pizza features a tomato base and falls under the category of 'pork pizzas.' Its delectable toppings include black pepper, cherry tomatoes, mozzarella di burrata, fior di latte mozzarella, olive oil, Parma ham, Parmesan cheese, and rocket."
        assert "La pizza 'BurraTadah' présente une base de tomate et appartient à la catégorie des pizzas au porc. Ses garnitures délicieuses comprennent du poivre noir, des tomates cerises, de la mozzarella di burrata, de la mozzarella fior di latte, de l'huile d'olive, du jambon de Parme, du fromage Parmesan et de la roquette." not in context_string

class TestBuildKG:

    def test_build_knowledge_graph_default_loader(self, default_rdf_graph_loader) -> None:
        kg_instance = default_rdf_graph_loader.build_knowledge_graph()

        test_endpoint_q = """
            SELECT ?label WHERE {
                <http://www.msesboue.org/o/pizza-data-demo/bisou#_blackPepper> rdfs:label ?label .
            }
        """
        test_q_res = [str(r["label"]) for r in kg_instance.sparql_endpoint(test_endpoint_q)]
